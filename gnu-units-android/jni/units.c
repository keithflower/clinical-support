#define VERSION "1.88_Android"  // NB: modified for Android port
 ...
 *     
 *  The original program was written by Adrian Mariano (adrian@cam.cornell.edu)
 *  This code has been modified to work on the Android platform, by Keith Flower (keith@keithflower.org)
 *
 */
...
static void logv(char *msg) {
	__android_log_print (ANDROID_LOG_VERBOSE, "native", msg);
}

static void logx(int level, char *tag, const char *format, ...) {
	va_list arglist;
...
char sUNITSFILE[1024];
char * smain (char *, char **, int);
#define ANDROID_ARGS_ERROR    999
#define ANDROID_HUGE_BUF_SIZE 88192
...
tryallunits(struct unittype *have, char *searchstring)
{
  ...
  if (!outfile)
    outfile = stdout;
#endif /* ifndef ANDROID_VERSION */
  for(i=0;i<count;i++){
#ifdef ANDROID_VERSION
    printf("%s %s\n", list[i].name, list[i].def);
  }
#else
...
\n\
    ...
    -V, --version       print version number and return\n\n\
...
findunitsfile()
{
  ...
  if (!testfile) {
    ...
    env = getenv("PATH");
    if (env) {
      filename = mymalloc(strlen(env)+strlen(sUNITSFILE)+2,"(findunitsfile)");
...
processargs(int argc, char **argv, char **from, char **to)
{
   ...
   while ( -1 != 
	 ...
	 case 'f':
	    ...
	    if (ind==MAXFILES){
	      printf("Flags/Options: At most %d -f specifications are allowed\n",
		      MAXFILES);
	      //fprintf(stderr, "At most %d -f specifications are allowed\n",
	      //      MAXFILES);
              return (ANDROID_ARGS_ERROR);
	    }
            if (optarg && *optarg)
   ...
   if (unitcheck) {
     if (optind != argc){
       ...
       // helpmsg();
       // Android
       printf("Flags/Options: Too many arguments (arguments are not allowed with -c).\n");
       return (ANDROID_ARGS_ERROR);
     }
   } else {
     if (optind == argc - 2) {
     ...
     if (optind < argc - 2) {
       ...
       // Android
       printf("Flags/Options: Too many arguments (maybe you need quotes).\n");
       return (ANDROID_ARGS_ERROR);
       // helpmsg();
     }
   }
...
char * smain (char *units_file_path, char **input, int argc) {
  static char huge_string_buf[8192];
  fpos_t pos;
  int i;


  memset(huge_string_buf, 0, sizeof(huge_string_buf));

  // In the android environment these globals will be maintained from  call to call so we have to reset here
  extern optind;
  optind = 0;
  numformat = "%.8g";	/* printf format for numeric output */
  powerstring = "^";	/* Exponent character used in output */
  quiet = 0;                  /* Flag for supressing prompting (-q option) */
  unitcheck = 0;              /* Flag for unit checking, set to 1 for
                                 regular check, set to 2 for verbose check */
  verbose = 1;                /* Flag for output verbosity */
  strictconvert = 0;          /* Strict conversion (disables reciprocals) */
  minusminus = 1;             /* Does '-' character give subtraction */
  oldstar = 0;                /* Does '*' have higher precedence than '/' */
  oneline = 0;                /* Suppresses the second line of output */

  strncpy (sUNITSFILE, units_file_path, sizeof(sUNITSFILE));

  // Capture everything sent to stdout. Note that for the Android
  // version we are discarding anything originally sent to stderr.
  fflush(stdout);
  fgetpos(stdout, &pos);
  ...
  // Call units (the erstwhile main() function)
  ...
  return (huge_string_buf);
}

char *getAllUnitNames()
{
  static char buf[ANDROID_HUGE_BUF_SIZE];
  memset(buf, 0, sizeof(buf));
  char *s = completeunits("", 0);
  sprintf (buf, "%s", s);
  while (1) {
     s = completeunits("", 1);
     if (s) {
...
#  endif
   ...
   // Android - don't try to recover from errors in arguments
   if (interactive == ANDROID_ARGS_ERROR) 
     return (ANDROID_ARGS_ERROR);

   // Android - the following is added to simulate the lack of an argument 
   // in non-interactive mode. The android front end always passes in empty strings instead
   // of null strings expected here.
   if (wantstr && !strncmp(wantstr, "", 1)) wantstr = 0;
   ...
   if (!unitsfiles[0]){
     ...
     if (unitsfiles[0])
       ...
       unitcount=0;
     unitsfiles[unitcount] = getenv("sUNITSFILE");
   ...
   for(unitfileptr=unitsfiles;*unitfileptr;unitfileptr++){
     ...
     if (readerr==E_MEMORY) 
       return (3);
     if (readerr==E_FILE){
       // Android - changed from output to stderr, so we can
       // pick up the case where a units file can't be opened
       printf("Flags/Options: unable to open units file '%s'.",
	       *unitfileptr);
       perror(0);
       return(1);
     }
   ...
   if (!interactive) {
     if (ishelpquery(havestr, 0)) return 0;
